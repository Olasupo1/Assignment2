    ...
    class Program
    {
        ...
        private static void ShuffleArray(int[] nums, int n)
        {
            try
            {
                if (nums.Length != n * 2)
                {
                    System.Console.WriteLine("Invalid input. Array length must be 2n");
                }
                // create an array of same capacity as the input array
                int[] shuffledArray = new int[nums.Length];
                for (int i = 0, j = 0; i < n; i++, j += 2)
                {
                    shuffledArray[j] = nums[i];
                    shuffledArray[j + 1] = nums[nums.Length / 2 + i];
                }
                
                for (int i = 0; i < shuffledArray.Length; i++)
                {
                    System.Console.Write(shuffledArray[i] + " ");
                }
       ...
       /// </summary>
        ...
        private static void CoolPairs(int[] nums)
        {
            try
            {
                ...
                {
                    // compare the current max value with num[i]
                    int test = nums[i];
        ...
        private static void TwoSum(int[] nums, int target)
        {
            ...
            {
                int len = nums.Length;
                // using diictionary for performance reasons
                ...
                for (int i = 0; i < len; i++)
                {
                    ...
                    {
                        Console.Write($"[{visited[lookingFor]}, {i}] ");
                    }
                    else
        ...
        private static void RestoreString(string s, int[] indices)
        {
            try
            {
                // convert variable 's' to character, so we can loop over them
        ...
        private static bool Isomorphic(string s1, string s2)
        {
            try
            {
                // using diction for performance reasons
                Dictionary<char, int> freqs = new Dictionary<char, int>();
                Dictionary<char, int> freqt = new Dictionary<char, int>();
                for (int i = 0; i < s1.Length; i++)
                {
                    if (!freqs.ContainsKey(s1[i]))
                        freqs.Add(s1[i], 0);
                    if (!freqt.ContainsKey(s2[i]))
                        freqt.Add(s2[i], 0);

                    if (freqs[s1[i]] != freqt[s2[i]])
                    {
                        // at this point, the 2 strings are not isomorphic
                        return false;
                    }
                ...
                // at this point, the 2 strings are isomorphic, so return true
                return true;
            }
            catch (Exception)
            {
        ...
        private static bool HappyNumber(int n)
        {
            try
            {
                // using HashSet for performace reasons
                ...
                while (n != 1)
                {
                    ...
                    // try to add variable 'nn' to the set, if iit faiils
                    // then the number alreeady exists, so we return 'False'
                    if (!hset.Add(nn))
                        return false;
                    n = nn;
                }
        ...
        private static int Stocks(int[] prices)
        {
            try
            {
                ...
                for (int i = 1; i < prices.Length; i++)
                {
                    ...
                    {
                        // check the maxValue before updating
                        maxProfit = Math.Max(maxProfit, maxValue - minValue);
                        // since the MaxValue is not Valid anymore has we have to buy before sell
                        maxValue = prices[i]; 
                        minValue = prices[i];
                    }
                }
